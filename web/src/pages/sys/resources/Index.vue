        ...
        <template #default="scope">
          <el-button-group>
            ...
            <el-button type="primary" icon="DocumentAdd" title="增加同级"
              @click="open(0, 'Edit', { parentId: scope.row.parentId })"></el-button>
            <el-button type="warning" icon="DocumentAdd" title="增加下级"
              @click="open(0, 'Edit', { parentId: scope.row.id })"></el-button>
          </el-button-group>
        </template>
      </el-table-column>
    </el-table>
...
export default {
  ...
  setup() {
    ...
    const { state, list, remove } = useList("/mgt/sys/resources")
